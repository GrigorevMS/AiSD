#pragma once
#include"TText.h"
#include"TStack.h"
#include"TTextLink.h"
#include<cstring>
#include<string>
#include<fstream>

// конструктор
TText::TText() {
	this->pFirst = nullptr;
	this->pCurrent = nullptr;
	this->pIter = nullptr;
}

// конструктор копирования
TText::TText(const TText& temp) {
	TTextLink* curr = temp.GetpFirst(); // итератор для temp (копируемого текста)
	TStack<TTextLink*> stack(100); // точки возврата из pDown
	TStack<TTextLink*> path(100); // весь путь
	bool flag_first = true;
	while (curr != nullptr) {
		// добавление нового элемента
		if (flag_first) {
			this->pFirst = new TTextLink(curr->GetStr());
			this->pCurrent = this->pFirst;
			flag_first = false;
		}
		else {
			if (path.See()->GetDown() == curr) {
				this->InsDownLink(new TTextLink(curr->GetStr()));
				this->GoDownLink();
			}
			else if (path.See()->GetNext() == curr) {
				this->InsNextLnk(new TTextLink(curr->GetStr()));
				this->GoNextLink();
			}
		}
		// переход к следующему элементу
		if (curr->GetDown() != nullptr) {
			path.Put(curr);
			stack.Put(curr);
			curr = curr->GetDown();
		}
		else if (curr->GetNext() != nullptr) {
			path.Put(curr);
			curr = curr->GetNext();
		}
		else {
			while (!stack.IsEmpty() && stack.See()->GetNext() == nullptr) {
				curr = stack.Get();
				this->GoUpLink();
			}
			if (stack.IsEmpty()) {
				curr = curr->GetNext();
				this->GoFirstLink();
			}
			else {
				curr = stack.Get();
				this->GoUpLink();
				path.Put(curr);
				curr = curr->GetNext();
			}
		}
	}
}

// конструктор преобразования типов
TText::TText(TTextLink* temp) {
	this->pFirst = temp;
	this->pCurrent = temp;
	this->pIter = temp;
}

// деструктор
TText::~TText() {
	this->GoFirstLink();
	while (this->pCurrent != nullptr)
		this->DelLink();
	this->Path.Clear();
	this->Up.Clear();
	this->pIter = nullptr;
	this->St.Clear();
}

// вернуть копию
TText TText::GetCopy() {
	TText copy(*this);
	return copy;
}

// вернуть указатель на первый
TTextLink* TText::GetpFirst() const {
	return this->pFirst;
}

// вернуть указатель на текущий
TTextLink* TText::GetpCurrent() const {
	return this->pCurrent;
}

// получить итератор
TTextLink* TText::GetIterator(void) const {
	return this->pIter;
}

/*// чтение текста из файла
TTextLink* TText::ReadText(std::ifstream& TxtFile) {

}*/


// навигация_________________________________________________________________________

// переход к первой строке
bool TText::GoFirstLink(void) {
	this->Path.Clear();
	this->pCurrent = this->pFirst;
	return true;
}

// переход к следующей строке по pDown
bool TText::GoDownLink(void) {
	if (this != nullptr) {
		if (this->pCurrent->GetDown() != nullptr) {
			this->Path.Put(this->pCurrent);
			this->Up.Put(this->pCurrent);
			this->pCurrent = this->pCurrent->GetDown();
			return true;
		}
		return false;
	}
	return false;
}

// переход к следующей строке по pNext
bool TText::GoNextLink(void) {
	
	if (this != nullptr) {
		if (this->pCurrent->GetNext() != nullptr) {
			this->Path.Put(this->pCurrent);
			this->pCurrent = this->pCurrent->GetNext();
			return true;
		}
		return false;
	}
	return false;
}

// переход к предыдущей позиции текста
bool TText::GoPrevLink(void) {
	if (!(this->Path.IsEmpty())) {
		this->Path.Output();
		if(this->Path.See()!=this->Up.See())
			this->pCurrent = this->Path.Get();
		else {
			this->pCurrent = this->Path.Get();
			this->Up.Get();
		}
		this->Path.Output();
		return true;
	}
	return false;
}

// переход к родителю
bool TText::GoUpLink(void) {
	if (this != nullptr) {
		if (!this->Up.IsEmpty()) {
			this->pCurrent = this->Up.Get();
			while (this->Path.See() != this->pCurrent)
				this->Path.Get();
			this->Path.Get();
			return true;
		}
		return false;
	}
	return false;
}


// модификация структуры текста______________________________________________________

// изъять текущий элемент (или его поддерево)
TTextLink* TText::GetLink(void) {
	this->Reset();
	TTextLink* temp = this->pCurrent; // запоминаем удаляемый элемент
	// вырезаем удаляемый из текста
	if (temp == this->pFirst) { // если вырезаем первый
		this->pFirst = temp->GetNext();
		this->pCurrent = this->pFirst;
		temp->SetNext(nullptr);
	}
	else { // если вырезаем не первый
		while (this->pIter->GetNext() != temp && this->pIter->GetDown() != temp) {
			this->GoNext();
		}
		if (this->pIter->GetNext() == temp) {
			this->pIter->SetNext(temp->GetNext());
			temp->SetNext(nullptr);
			this->pCurrent = this->pIter->GetNext();
			if (this->pCurrent == nullptr)
				this->pCurrent = this->pFirst;
		}
		else if (this->pIter->GetDown() == temp) {
			this->pIter->SetDown(temp->GetNext());
			temp->SetNext(nullptr);
			this->pCurrent = this->pIter->GetDown();
			if (this->pCurrent == nullptr)
				this->pCurrent = this->pFirst;
		}
	}
	return temp;
}

// удалить текущий элемент (или его поддерево)
void TText::DelLink(void) {
	this->Reset();
	TTextLink* temp = this->pCurrent; // запоминаем удаляемый элемент
	// вырезаем удаляемый из текста
	if (temp == this->pFirst) { // если вырезаем первый
		this->pFirst = temp->GetNext();
		this->pCurrent = this->pFirst;
		temp->SetNext(nullptr);
	}
	else { // если вырезаем не первый
		while (this->pIter->GetNext() != temp && this->pIter->GetDown() != temp) {
			this->GoNext();
		}
		if (this->pIter->GetNext() == temp) {
			this->pIter->SetNext(temp->GetNext());
			temp->SetNext(nullptr);
			this->pCurrent = this->pIter->GetNext();
			if (this->pCurrent == nullptr)
				this->pCurrent = this->pFirst;
		}
		else if (this->pIter->GetDown() == temp) {
			this->pIter->SetDown(temp->GetNext());
			temp->SetNext(nullptr);
			this->pCurrent = this->pIter->GetDown();
			if (this->pCurrent == nullptr)
				this->pCurrent = this->pFirst;
		}
	}
	// удаляем из динамической памяти вырезанный элемент и его поддерево
	this->DelSubTree(temp);
}

// рекурсия удаления поддерева
void TText::DelSubTree(TTextLink* pTl) {
	if (pTl->GetDown() != nullptr) this->DelSubTree(pTl->GetDown());
	TTextLink* temp = pTl->GetNext();
	delete pTl;
	if (temp != nullptr) this->DelSubTree(temp);
}

// вставить элемент pl (или его поддерево) следующим за текущим
void TText::InsNextLnk(TTextLink* pl) {
	pl->SetNext(this->pCurrent->GetNext());
	this->pCurrent->SetNext(pl);
}

// вставить элемент pl (или его поддерево)перед теку-щим
void TText::InsPrevLnk(TTextLink* pl) {
	TTextLink* prev = this->Path.See();
	if (!this->Path.IsEmpty()) {
		if (prev->GetNext() == this->pCurrent) {
			pl->SetNext(this->pCurrent);
			prev->SetNext(pl);
		}
		else if (prev->GetDown() == this->pCurrent) {
			pl->SetNext(this->pCurrent);
			prev->SetDown(pl);
		}
	}
	else {
		pl->SetNext(this->pCurrent);
		this->pFirst = pl;
	}
}

// вставить элемент pd (или его поддерево) вниз за текущим
void TText::InsDownLink(TTextLink* pd) {
	pd->SetNext(this->pCurrent->GetDown());
	this->pCurrent->SetDown(pd);
}

// вставить элемент pd (или его поддерево) вверх перед текущим
void TText::InsUpLink(TTextLink* pd) {
	TTextLink* prev = this->Path.See();
	if (!this->Path.IsEmpty()) {
		pd->SetDown(this->pCurrent);
		pd->SetNext(this->pCurrent->GetNext());
		this->pCurrent->SetNext(nullptr);
		if (prev->GetNext() == this->pCurrent)
			prev->SetNext(pd);
		else if (prev->GetDown() == this->pCurrent)
			prev->SetDown(pd);
		this->pCurrent = pd;
	}
	else if (this->pCurrent == this->pFirst) {
		pd->SetDown(this->pCurrent);
		pd->SetNext(this->pCurrent->GetNext());
		this->pCurrent->SetNext(nullptr);
		this->pFirst = pd;
		this->pCurrent = pd;
	}
}


// доступ____________________________________________________________________________

// чтение текущей строки 
std::string TText::GetLine(void) {
	if (this->pCurrent == nullptr)
		return "Can't read Str, because of nullptr";
	return this->pCurrent->Str;
}

// замена текущей строки 
void TText::SetLine(string s) {
	this->pCurrent->SetStr(s);
}


// итератор__________________________________________________________________________

// установить на первую запись 
void TText::Reset(void) {
	this->St.Clear();
	this->pIter = this->pFirst;
}

// текст завершен? 
bool TText::IsTextEnded(void) const {
	if (this->pIter == nullptr)
		return true;
	return false;
}

// переход к следующей записи 
bool TText::GoNext(void) {
	if (this->pIter != nullptr) {
		if (this->pIter->GetDown() != nullptr) {
			this->St.Put(this->pIter);
			this->pIter = this->pIter->GetDown();
		}
		else if (this->pIter->GetNext() != nullptr) {
			this->pIter = this->pIter->GetNext();
		}
		else {
			while (!(this->St.IsEmpty()) && this->St.See()->GetNext() == nullptr) {
				this->pIter = this->St.Get();
			}
			if (this->St.IsEmpty()) {
				this->pIter = this->pIter->GetNext();
			}
			else {
				this->pIter = this->St.Get();
				this->pIter = this->pIter->GetNext();
			}
		}
		return true;
	}
	else {
		return false;
	}
	/*if (this->pCurrent != nullptr) {
		if (this->pCurrent->GetDown() != nullptr) {
			this->Path.Put(this->pCurrent);
			this->St.Put(this->pCurrent);
			this->pCurrent = this->pCurrent->GetDown();
			return true;
		}
		else if (this->pCurrent->GetNext() != nullptr) {
			this->Path.Put(this->pCurrent);
			this->pCurrent = this->pCurrent->GetNext();
			return true;
		}
		else {
			while (!(this->St.IsEmpty()) && this->St.See()->GetNext() == nullptr) {
				this->pCurrent = this->St.Get();
			}
			if (this->St.IsEmpty()) {
				this->pCurrent = this->pCurrent->GetNext();
				return true;
			}
			else {
				this->pCurrent = this->St.Get();
				this->pCurrent = this->pCurrent->GetNext();
				return true;
			}
		}
	}
	else {
		//cout << "Can't Go next\n";
		return false;
	}*/
	/*bool result = true;
	if (this->pCurrent != nullptr) {
		//this->St.Output();
		if (this->pCurrent->GetDown() != nullptr) {
			this->St.Put(this->pCurrent);
			this->Path.Put(this->pCurrent);
			this->pCurrent = this->pCurrent->GetDown();
			result = true;
		}
		else if (this->pCurrent->GetNext() != nullptr) {
			this->Path.Put(this->pCurrent);
			this->pCurrent = this->pCurrent->GetNext();
			result = true;
		}
		else {
			while (!(this->St.IsEmpty()) && this->St.See()->GetNext() == nullptr) {
				this->pCurrent = this->St.Get();
			}
			if (this->St.IsEmpty()) {
				this->Reset();
				result = false;
			}
			else {
				this->pCurrent = this->St.Get();
				this->pCurrent = this->pCurrent->GetNext();
				result = true;
			}
		}
	}
	else {
		this->Reset();
		result = false;
	}
	return result;*/
}


// Работа с файлами__________________________________________________________________

// ввод текста из файла 
void TText::Read(std::string pFileName) {
	fstream fin;
	fin.open(pFileName);
	std::string temp;
	TStack<TTextLink*> stack(100);
	bool flag_first = true;
	while (getline(fin, temp)) {
		TTextLink* add = new TTextLink(temp);
		if (flag_first) {
			this->pFirst = add;
			this->pCurrent = add;
			flag_first = false;
			stack.Put(add);
		}
		else {
			if (temp == "{") {
				TTextLink* prev = stack.See();
				getline(fin, temp);
				TTextLink* add = new TTextLink(temp);
				prev->SetDown(add);
				stack.Put(add);
			}
			else if (temp == "}") {
				stack.Get();
			}
			else {
				TTextLink* prev = stack.Get();
				prev->SetNext(add);
				stack.Put(add);
			}
		}
	}
	this->Reset();
	fin.close();
}

// сохранение текста в файл
void TText::Write(std::string pFileName) {
	ofstream fout;
	fout.open(pFileName);
	TTextLink* curr = this->pFirst;
	TStack<TTextLink*> stack(100);

	while (curr != nullptr) {
		fout << curr->GetStr() << '\n';
		if (curr->GetDown() != nullptr) {
			fout << "{\n";
			stack.Put(curr);
			curr = curr->GetDown();
		}
		else if (curr->GetNext() != nullptr) {
			curr = curr->GetNext();
		}
		else {
			while (!stack.IsEmpty() && stack.See()->GetNext() == nullptr) {
				curr = stack.Get();
				fout << "}\n";
			}
			if (stack.IsEmpty()) {
				curr = curr->GetNext();
			}
			else {
				curr = stack.Get();
				fout << "}\n";
				curr = curr->GetNext();
			}
		}
	}
	fout.close();

	/*ofstream fout;
	TTextLink* prev;
	std::string tab;
	fout.open(pFileName);
	this->Reset();
	fout << this->pIter->GetStr() << '\n';
	prev = this->pIter;
	while (this->GoNext() && this->pIter!=nullptr) {
		tab = "";
		if (prev->GetDown() == this->pIter) {
			fout << "{" << '\n';
		}
		else if (prev->GetDown() != this->pIter && prev->GetNext() != this->pIter) {
			fout << "}" << '\n';
		}
		fout << this->pIter->GetStr() << '\n';
		prev = this->pIter;
	}*/
}


// Печать____________________________________________________________________________

// печать текста 
void TText::Print(void) {
	TTextLink* curr = this->pFirst;
	TStack<TTextLink*> stack(100);
	int level = 0;
	while (curr != nullptr) {
		std::string tab = "";
		for (int i = 0; i < level; i++)
			tab = tab + "    ";
		cout << tab << curr->GetStr();
		if (curr == this->pCurrent)
			cout << "(+)";
		cout << endl;
		if (curr->GetDown() != nullptr) {
			stack.Put(curr);
			curr = curr->GetDown();
			level++;
		}
		else if (curr->GetNext() != nullptr) {
			curr = curr->GetNext();
		}
		else {
			while (!(stack.IsEmpty()) && stack.See()->GetNext()==nullptr) {
				curr = stack.Get();
				level--;
			}
			if (stack.IsEmpty()) {
				curr = curr->GetNext();
			}
			else {
				curr = stack.Get();
				level--;
				curr = curr->GetNext();
			}
		}
	}
}

// печать текста со звена ptl (рекурсия)
void TText::PrintText(TTextLink* ptl, int level) {
	string temp = "";
	for (int i = 0; i < level; i++) {
		temp = temp + "    ";
	}
	cout << temp << ptl->GetStr();
	if (ptl == this->pCurrent) {
		cout << "(+)";
	}
	cout << '\n';
	if (ptl->GetDown() != nullptr)	this->PrintText(ptl->GetDown(), level + 1);
	if (ptl->GetNext() != nullptr)	this->PrintText(ptl->GetNext(), level);
}

TText& TText::operator=(const TText& temp)
{
	// предварительная очистка
	this->GoFirstLink();
	while (this->pCurrent != nullptr)
		this->DelLink();
	this->Path.Clear();
	this->Up.Clear();
	this->pIter = nullptr;
	this->St.Clear();

	// перезапись
	TTextLink* curr = temp.GetpFirst(); // итератор для temp (копируемого текста)
	TStack<TTextLink*> stack(100); // точки возврата из pDown
	TStack<TTextLink*> path(100); // весь путь
	bool flag_first = true;
	while (curr != nullptr) {
		// добавление нового элемента
		if (flag_first) {
			this->pFirst = new TTextLink(curr->GetStr());
			this->pCurrent = this->pFirst;
			flag_first = false;
		}
		else {
			if (path.See()->GetDown() == curr) {
				this->InsDownLink(new TTextLink(curr->GetStr()));
				this->GoDownLink();
			}
			else if (path.See()->GetNext() == curr) {
				this->InsNextLnk(new TTextLink(curr->GetStr()));
				this->GoNextLink();
			}
		}
		// переход к следующему элементу
		if (curr->GetDown() != nullptr) {
			path.Put(curr);
			stack.Put(curr);
			curr = curr->GetDown();
		}
		else if (curr->GetNext() != nullptr) {
			path.Put(curr);
			curr = curr->GetNext();
		}
		else {
			while (!stack.IsEmpty() && stack.See()->GetNext() == nullptr) {
				curr = stack.Get();
				this->GoUpLink();
			}
			if (stack.IsEmpty()) {
				curr = curr->GetNext();
				this->GoFirstLink();
			}
			else {
				curr = stack.Get();
				this->GoUpLink();
				path.Put(curr);
				curr = curr->GetNext();
			}
		}
	}

	// возврат
	return *this;
}